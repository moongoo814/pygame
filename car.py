...
def draw_main_menu():
    ...
    text_title = font_40.render("Pycar: Racing Car Game", True ,False, BLACK)
    screen.bilt(text_title, [draw_x,draw_y])
    text_score = font_40.render("Score:" + str(score), True,WHITE)
    screen.bilt(text_score, [draw_x, draw_y + 70])
    text_start = font_30.render("Press Space Key to Start",True, RED)
    screen.bilt(text_start, [draw_x, draw_y + 140])
    pygame.display.flip()
        ...
        for event in pygame.event.get():
            ...
            if crash:
                if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE:
                    ...
                    player.dx = 0
                    score = 0
                    pygame.mouse_set_visible(False)
                    sound_engine.play()
                    sleep(5)
